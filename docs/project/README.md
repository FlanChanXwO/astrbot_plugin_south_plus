...
自动签到订阅推送会在标题区分范围：全局订阅显示 `South Plus 自动签到（全局订阅）`，会话订阅显示 `South Plus 自动签到（会话订阅）`。正文按参与账号数、完成账号数、日签/周签三态统计输出；本次新签成功计入 `✅`，站点已签或本地缓存已完成计入 `⏭️`，失败计入 `❌`。日签和周签均未失败的账号计入完成，任一维度失败时仍会按原逻辑 @ 对应账号。
