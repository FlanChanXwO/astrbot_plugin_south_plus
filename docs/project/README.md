...
- `/splogin` — 生成或复用一次性临时登录链接；同平台同用户未过期时复用原 token，不顺延有效期，在网页表单完成账号/密码/验证码提交。首次绑定成功时若插件开启了自动签到，会附加提示。
...
- `/spcheckinallsub`（别名 `/sp全局签到订阅`）— 管理员订阅或取消当前会话的**全局**签到汇报推送；全局统计所有会话订阅参与账号的 UID 并集，无会话订阅时回退为全部启用账号。
...
自动签到订阅推送会在标题区分范围：全局订阅显示 `South Plus 自动签到（全局订阅）`，会话订阅显示 `South Plus 自动签到（会话订阅）`。会话订阅只统计当前会话订阅账号；全局订阅统计所有会话订阅参与账号的 UID 并集，同一 UID 多会话订阅只算一次，无会话订阅时回退为全部启用账号。正文按参与账号数、完成账号数、日签/周签三态统计输出；本轮有效签到流程经 verify 确认完成、或本地可信 `success` 缓存计入 `✅`，本轮开始前站点已签或本地可信 `already_done` 缓存计入 `⏭️`，失败计入 `❌`。日签和周签均未失败的账号计入完成，任一维度失败时仍会按原逻辑 @ 对应账号。
...
调度任务页的“参与账号”采用会话级排除语义：排除某 UID 只写入 `checkin_session_exclusion(umo, sp_uid)`，只影响该会话贡献的自动签到参与关系；不修改 `user.auto_checkin`，也不影响同一 UID 在其他会话继续参与或恢复。全局订阅汇总时，只要某 UID 在任一会话仍有效参与，就会纳入全局统计。账号页的自动签到按钮如使用，语义是用户账号自己的全局开关。
