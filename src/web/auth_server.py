...
from ..core.datamodels import AuthServerConfig, CredentialSession
from ..utils import expires_at_after, generate_token, season_name
...
def _seasonal_logo(now: datetime.datetime | None = None) -> str:
    ...
    """
    return _SEASONAL_LOGOS[season_name(now)]


def _render(template_name: str, **mapping: str) -> str:
    """从 static/ 读取模板，按 string.Template 语法替换占位符。模板首次加载后缓存。"""
