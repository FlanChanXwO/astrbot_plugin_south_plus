...
from ..exceptions import SouthPlusLoginError
...
from ..models import CaptchaPayload, LoginRequest, LoginResult

__all__ = ["SouthPlusLoginApi", "SouthPlusLoginAttempt"]


class SouthPlusLoginApi:
    ...
    def new_attempt(self) -> SouthPlusLoginAttempt:
        return SouthPlusLoginAttempt(self.session)
    ...
    def check_cookie(self, cookie: str) -> str:
        ...
        headers = {
            "Cookie": cookie,
            "Referer": self.session.endpoints.site_base_url + "/",
        }
        response = self.session.client.get(
            self.session.endpoints.verify_url,
            headers=headers,
        )
        body = response.text
...
class SouthPlusLoginAttempt:
    ...
    内部用 ``session.create_isolated_client()`` 创建独立 client，
    因为登录流程需要自己的 cookie jar（captcha + submit 跨请求保持）。
    """

    def __init__(self, session: SouthPlusSession) -> None:
        self.session = session
        self._client = session.create_isolated_client(
    ...
    def fetch_captcha(self) -> CaptchaPayload:
        ...
        response = self._client.get(
            self.session.endpoints.captcha_url,
            params={"nowtime": nowtime},
            headers={
                "Referer": self.session.endpoints.login_url,
    ...
    def submit(self, request: LoginRequest) -> LoginResult:
        ...
        payload = {
            ...
            "jumpurl": self.session.endpoints.site_base_url + "/" + DEFAULT_VERIFY_PATH,
        ...
        response = self._client.post(
            self.session.endpoints.login_url,
            data=payload,
            headers={
                "Referer": self.session.endpoints.login_url,
                "Origin": self.session.endpoints.site_base_url,
                "Content-Type": "application/x-www-form-urlencoded",
            },
        )
        body = response.text
        cookie_header = _cookie_header(
            self._client, self.session.endpoints.cookie_domains
        )
    ...
    def _ensure_login_page(self) -> None:
        ...
        response = self._client.get(
            self.session.endpoints.login_url,
            headers={"Referer": self.session.endpoints.site_base_url + "/"},
        )
        response.raise_for_status()
